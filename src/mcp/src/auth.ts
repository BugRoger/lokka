...
// Token file structure for persistent token caching
 ...
 */
export class PersistentTokenCredential implements TokenCredential {
  private clientId: string;
  private redirectPort: number;
  private cachedToken: TokenFileData | null = null;

  constructor(clientId: string = LokkaClientId, redirectPort: number = 0) {
  ...
  private _loadTokenFile(): TokenFileData | null {
    try {
      if (!fs.existsSync(LokkaTokenPath)) {
        return null;
      }
      const data = fs.readFileSync(LokkaTokenPath, "utf-8");
      const parsed = JSON.parse(data) as TokenFileData;
  ...
  private _saveTokenFile(data: TokenFileData): void {
  ...
  private async _refreshToken(): Promise<TokenFileData | null> {
  ...
  private async _interactiveAuth(): Promise<TokenFileData> {
    ...
    return new Promise<TokenFileData>((resolve, reject) => {
  ...
  async initialize(): Promise<void> {
    switch (this.config.mode) {
      ...
      case AuthMode.PersistentToken:
        ...
        this.credential = new PersistentTokenCredential(
          this.config.clientId || LokkaClientId,
        );
        break;
